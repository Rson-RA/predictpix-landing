...
const LAUNCH_AT_ISO = "2025-09-30T12:00:00+09:00";
...
function TimeBox({ n, label }) {
  return (
    <div className="flex flex-col items-center">
      <div className="px-3 py-2 md:px-4 md:py-3 rounded-xl bg-black/30 ring-1" style={{ borderColor: COLORS.borderSoft }}>
        <span className="tabular-nums text-3xl md:text-4xl font-semibold">{String(n).padStart(2, "0")}</span>
      </div>
...
function GradientButton({ children, href, onClick, variant = "pink" }) {
  const gradient =
    variant === "pink"
  ...
  const cls = "inline-flex items-center justify-center rounded-2xl px-5 py-3 font-semibold text-black shadow-lg hover:opacity-95 transition";
...
function Section({ title, children, id }) {
  return (
    <section id={id} className="w-full max-w-6xl mx-auto px-5 md:px-8 py-12 md:py-16">
      <h2 className="text-2xl md:text-3xl font-bold mb-4">{title}</h2>
      <div className="text-white/80 leading-relaxed text-base md:text-lg">{children}</div>
    </section>
  );
}
  ...
  return (
    <div style={{ backgroundColor: COLORS.bg, color: COLORS.text }} className="min-h-screen w-full">
      <div
        ...
        style={{
          background:
            `radial-gradient(600px 400px at 15% 10%, ${COLORS.accentPink}22 0%, transparent 70%),
      ...
      <header className="w-full">
        <div className="max-w-6xl mx-auto flex items-center justify-between px-5 md:px-8 py-5">
          <div className="flex items-center gap-3">
            <div className="w-9 h-9 rounded-xl" style={{
              background: `linear-gradient(30deg, ${COLORS.accentCyan}, ${COLORS.accentPink})`,
              boxShadow: "0 0 24px rgba(255,46,207,0.35)",
            }} />
            <div className="text-xl font-bold tracking-tight">Predict<span style={{ color: COLORS.accentOrange }}>PiX</span></div>
          </div>
          <nav className="hidden md:flex items-center gap-6 text-white/80">
            <a href="#how" className="hover:text-white">How it works</a>
            <a href="#perks" className="hover:text-white">Beta perks</a>
            <a href="#roadmap" className="hover:text-white">Roadmap</a>
            <a href="#faq" className="hover:text-white">FAQ</a>
          </nav>
          <div className="flex items-center gap-3">
            <a href={preLaunch ? "#waitlist" : "/app"} className="hidden md:inline-block text-sm px-4 py-2 rounded-xl ring-1 hover:bg-white/5" style={{ borderColor: COLORS.borderSoft }}>
      ...
      <main className="w-full">
          ...
          <div className="grid md:grid-cols-2 gap-8 items-center">
            <div>
              <h1 className="text-4xl md:text-6xl font-extrabold leading-tight">
                Prediction Markets <span className="block mt-1">Powered by <span style={{ color: COLORS.accentOrange }}>Pi</span></span>
              </h1>
              ...
              {preLaunch ? (
                <div className="mt-6">
                  ...
                  <div className="flex flex-wrap gap-3 mt-6">
                    <GradientButton variant="pink" href="#waitlist">Join the Waitlist</GradientButton>
                    <GradientButton variant="orange" href="/login?next=/beta">Join Beta with Pi Login</GradientButton>
                  </div>
                  <p className="text-sm text-white/60 mt-3">Use your referral link <span className="text-white">?ref=YOURCODE</span> to earn early-access boosts.</p>
                </div>
              ) : (
                <div className="mt-6">
                  <div className="text-white/80">We are live!</div>
                  <div className="flex flex-wrap gap-3 mt-4">
                    <GradientButton variant="pink" href="/app">Open PredictPiX</GradientButton>
                    <a href="#stats" className="px-5 py-3 rounded-2xl ring-1 text-white/90 hover:bg-white/5" style={{ borderColor: COLORS.borderSoft }}>See live stats</a>
                  </div>
                </div>
              )}
            </div>

            <div className="rounded-3xl p-6 md:p-8 shadow-xl backdrop-blur"
                 style={{ background: "linear-gradient(180deg, rgba(255,255,255,0.06), rgba(255,255,255,0.02))", border: `1px solid ${COLORS.borderSoft}` }}>
              {preLaunch ? (
                <div>
                  <h3 className="text-2xl font-semibold">Get Early Access</h3>
                  <p className="text-white/70 mt-2">Be among the first 1,000 to enter beta and claim a Founding Predictor badge.</p>
                  ...
                  <form id="waitlist" onSubmit={submit} className="mt-5 grid gap-3">
                    ...
                    <label className="text-sm text-white/80 mt-2" htmlFor="wallet">Pi Wallet (optional)</label>
                    <input
                      id="wallet"
                      type="text"
                      value={wallet}
                    ...
                    <div className="flex items-start gap-2 mt-2 text-sm text-white/70">
                      ...
                      <label htmlFor="consent">I agree to receive beta updates and I’ve read the <a className="underline" href="/privacy" target="_blank" rel="noreferrer">Privacy Policy</a>.</label>
                    </div>

                    <div className="flex gap-3 mt-3">
                      <GradientButton variant="pink">Join Waitlist</GradientButton>
                      <a href="/login?next=/beta" className="px-5 py-3 rounded-2xl ring-1 hover:bg-white/5" style={{ borderColor: COLORS.borderSoft }}>Pi Login</a>
                    </div>

                    {message && (
                      <div className="text-sm mt-3" style={{ color: message.startsWith("Signup failed") || message.startsWith("Network") ? "#ffb4b4" : "#C6FFD0" }}>{message}</div>
                    )}
                ...
                <div id="stats" className="grid gap-6">
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
                    ...
                    <Stat label="Total Traders" value={2391} />
                    <Stat label="24h Volume (Pi)" value={"12,540"} />
                    <Stat label="Markets Resolved" value={76} />
                  </div>
                  <div className="flex gap-3 mt-2">
                    <GradientButton variant="orange" href="/app">Start Trading</GradientButton>
                    <a href="#how" className="px-5 py-3 rounded-2xl ring-1 hover:bg-white/5" style={{ borderColor: COLORS.borderSoft }}>How it works</a>
                  </div>
                </div>
              )}
            </div>
        ...
        <Section id="how" title="How PredictPiX Works">
          <ul className="grid md:grid-cols-3 gap-6 list-none p-0">
            {[
              ...
              <li key={i} className="rounded-2xl p-5" style={{ background: "rgba(255,255,255,0.04)", border: `1px solid ${COLORS.borderSoft}` }}>
        ...
        <Section id="perks" title="Beta Access & Perks">
          <div className="grid md:grid-cols-3 gap-6">
            {[
              ...
              <div key={i} className="rounded-2xl p-5" style={{ background: "rgba(255,255,255,0.04)", border: `1px solid ${COLORS.borderSoft}` }}>
          ...
          <div className="flex gap-3 mt-6">
            ...
            <a href="/influencers" className="px-5 py-3 rounded-2xl ring-1 hover:bg-white/5" style={{ borderColor: COLORS.borderSoft }}>Apply as Influencer</a>
          </div>
        </Section>
      ...
      <footer className="w-full mt-8 border-t" style={{ borderColor: COLORS.borderSoft }}>
        <div className="max-w-6xl mx-auto px-5 md:px-8 py-10 grid md:grid-cols-2 gap-6 items-center">
          <div className="text-white/75 text-sm">
            © {new Date().getFullYear()} PredictPiX • All rights reserved.
          </div>
